...
LEFT JOIN [CreatorsBoardgames]
 ...
 --09. Creators with Emails
  SELECT CONCAT([c].[FirstName],' ',[c].[LastName])
      AS [FullName],[c].[Email],MAX([b].[Rating])
    FROM [Creators]
      AS [c]
   ...
   WHERE SUBSTRING([Email], CHARINDEX('.',[Email]),4) = '.com'
GROUP BY [c].[FirstName],[c].[LastName],[c].[Email]
ORDER BY [FullName] ASC

--10. Creators by Rating

  SELECT [c].[LastName],CEILING(AVG([b].[Rating]))
      AS [AverageRating],[p].[Name]
	  AS [PublisherName]
    FROM [Creators]
      AS [c]
    JOIN [CreatorsBoardgames]
...
CREATE PROC usp_SearchByCategory (@category VARCHAR(50))
         AS 
	  BEGIN
			SELECT [b].[Name],[b].[YearPublished],[b].[Rating],[c].[Name]
			    AS [CategoryName],[p].[Name]
				AS [PublisherName],CONCAT([pr].[PlayersMin],' ', 'people')
				AS [MinPlayers],CONCAT([pr].[PlayersMax],' ', 'people')
				AS [MaxPlayers]
			  FROM [Boardgames]
				AS [b]
			  JOIN [Categories]
